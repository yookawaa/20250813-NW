const app = new Vue({
  ...
  methods: {
    ...
    addData: async function() {
      ...
      //INSERT用のAPIを呼び出し
      const response = await axios.post('https://functionappapi20250813.azurewebsites.net/api/INSERT?',param);
    ...
    readData: async function() {
      //SELECT用のAPIを呼び出し      
      const response = await axios.get('https://functionappapi20250813.azurewebsites.net/api/SELECT?');
